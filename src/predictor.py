...
class Predictor:
    ...
    def img_preprocess(self):
        return SingleNail(self.img_path, augmentation=get_validation_augmentation(),
                          preprocessing=get_preprocessing(self.preprocessing_fn),
                          class_rgb_values=self.select_class_rgb_values)[0]
    ...
    def get_visualize(self, pred_mask):
        ...
        image_vis = SingleNail(self.img_path, augmentation=get_validation_augmentation(),
                               class_rgb_values=self.select_class_rgb_values)[0]
        image_vis = Predictor.crop_image(image_vis.astype('uint8'))
        pred_mask = Predictor.crop_image(
            colour_code_segmentation(reverse_one_hot(pred_mask), self.select_class_rgb_values))
